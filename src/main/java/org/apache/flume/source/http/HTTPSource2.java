 ...
 *
 * NOTE: This file was modified from it's original format to meet the needs
 * of this project.
 */
 ...
 *
 */
public class HTTPSource2 extends AbstractSource implements
   ...
   */
	...
	private static final Logger LOG = LoggerFactory.getLogger(HTTPSource2.class);
	...
	public void configure(Context context) {
		try {
			...
			Preconditions.checkState(host != null && !host.isEmpty(),
					"HTTPSource2 hostname specified is empty");
			Preconditions.checkNotNull(port, "HTTPSource2 requires a port number to be"
					+ " specified");
			...
			LOG.error("Error while configuring HTTPSource2. Exception follows.", ex);
			...
			LOG.error("Error configuring HTTPSource2!", ex);
			Throwables.propagate(ex);
		}
	...
	public void start() {
		...
		try {
			...
			root.addServlet(new ServletHolder(new FlumeHTTPServlet(new FlumeHTTPContextImplementation())), "/");
			HTTPServerConstraintUtil.enforceConstraints(root);
			srv.start();
			Preconditions.checkArgument(srv.getHandler().equals(root));
		} catch (Exception ex) {
			LOG.error("Error while starting HTTPSource2. Exception follows.", ex);
			Throwables.propagate(ex);
		}
	...
	public void stop() {
		try {
			...
			LOG.error("Error while stopping HTTPSource2. Exception follows.", ex);
		}
		sourceCounter.stop();
		LOG.info("Http source {} stopped. Metrics: {}", getName(), sourceCounter);
	}

	private interface FlumeHTTPContext {
		HTTPSourceHandler getHandler();
		SourceCounter getSourceCounter();
		ChannelProcessor getUnderlyingChannelProcessor();
	}

	private class FlumeHTTPContextImplementation implements FlumeHTTPContext {
		@Override
		public HTTPSourceHandler getHandler() {
			return handler;
		}
		...
		@Override
		public ChannelProcessor getUnderlyingChannelProcessor() {
			return getChannelProcessor();
		}
	}

	private static class FlumeHTTPServlet extends HttpServlet {

		private static final long serialVersionUID = 4891924863218790344L;

		private final FlumeHTTPContext flumeContext;

		public FlumeHTTPServlet(FlumeHTTPContext flumeHTTPContext) {
			this.flumeContext = flumeHTTPContext;
		}
		...
		public void doPost(HttpServletRequest request, HttpServletResponse response)
			...
			try {
				events = flumeContext.getHandler().getEvents(request);
			...
			flumeContext.getSourceCounter().incrementAppendBatchReceivedCount();
			flumeContext.getSourceCounter().addToEventReceivedCount(events.size());
			try {
				flumeContext.getUnderlyingChannelProcessor().processEventBatch(events);
			...
			flumeContext.getSourceCounter().incrementAppendBatchAcceptedCount();
			flumeContext.getSourceCounter().addToEventAcceptedCount(events.size());
		}
